...
//Insert Customer
module.exports.startGame=(customerName, customerPhone, customerAddress)=>
{
  let db = new sqlite3.Database('./db/breakers.db', sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
    if (err) {
      console.error(err.message);
  ...
  return new Promise(function(resolve, reject) {
    db.run('Insert into customer ( customerName, customerPhone ,  customerAddress) values (?,?,?)', [customerName, customerPhone ,  customerAddress], (err) => {
        if (err !== null) 
        reject(err);
        else 
        {
